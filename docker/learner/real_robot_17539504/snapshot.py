...
out=stage/'output'
ckpt=out/'pick-r2-a40x2-b8-utd20-s42-j17539504/checkpoints'
remote=Path('/iris/u/kevinon/outputs/expo-ft-real/17539504')
remote.mkdir(parents=True,exist_ok=True)
records=sorted(ckpt.glob('robot-*/buffers/*.pkl'))
steps=sorted(int(p.name) for p in ckpt.iterdir() if p.is_dir() and p.name.isdigit() and (ckpt/f'round-{p.name}.json').exists()) if ckpt.exists() else []
...
marker=stage/'last-snapshot.json'
...
with tarfile.open(archive,'w') as tar:
    ...
    if latest is not None:
        for p in [ckpt/str(latest),ckpt/f'round-{latest}.json']:
