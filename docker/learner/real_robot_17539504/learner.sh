...
RESULTS=/iris/u/kevinon/outputs/expo-ft-real/17539504
...
cleanup() {
 ...
 python3 "$SNAPSHOT/snapshot.py" || true
 cp "$STAGE/output/learner.log" "$RESULTS/learner.log" || true
...
(while sleep 600; do python3 "$SNAPSHOT/snapshot.py"; done) > "$STAGE/output/backup.log" 2>&1 &
backup_pid=$!
printf 'Starting learner in job %s on %s; logs %s\n' "$SLURM_JOB_ID" "$(hostname)" "$STAGE/output/learner.log"
"${container[@]}" python -u train_pi_robo.py \
 ...
 --output_dir=/scr/kevinon/data/expo-real-17539504 --run_name=pick-r2-a40x2-b8-utd20-s42-j17539504 \
 --project_name=mtexpo --resume --checkpoint_model --checkpoint_buffer --checkpoint_interval=1000 \
 > "$STAGE/output/learner.log" 2>&1 &
child_pid=$!
set +e
wait "$child_pid"
code=$?
